...
<script>
import { registerModel } from '../service/index'
import { isRegister } from '../service/index'
export default {
    name: 'MyRegister',
    props: ['register'],
    data() {
        ...
        let validateName = (rule, value, callback) => {
            ...
            if (userNameRule.test(value)) {
                // 判断数据库中是否存在该用户名
                isRegister(this.RegisterUser.name).then(res => {
                    // code 为001是表示可以注册
    ...
    methods: {
        Register() {
            ...
            this.$refs['ruleForm'].validate(valid => {
                // 通过验证时开始注册
                if (valid) {
                    registerModel(this.RegisterUser.name, this.RegisterUser.pass).then(res => {
                        // code 为001时注册成功
                        // 注册成功关闭注册组件
